            ...
            foreach ((DialogRecord dia, List<Dialog.DialogInfoRecord> infos) in dialog)
            {
                ...
                foreach (DialogInfoRecord info in infos)
                {
                    ...
                    for (int i=0;i<lines.Count();i++)
                    {
                        ...
                        string hashName = $"{info.text.GetMD5Hash() }_{info.sex.ToString()}+{i}"; // Get the hash of the actual text string for this line, it will be our unique identier and filename for the cached wav/wem
